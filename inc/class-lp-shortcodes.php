...
class LP_Shortcodes {
    ...
    public static function init() {
        $shortcodes = array(
            ...
            'learn_press_attendance' => __CLASS__ . '::attendance'
        );
    ...
    public static function auto_shortcode($template) {
        if (is_page()) {
            ...
            } elseif ($page_id == learn_press_get_page_id('attendance')) {
                if (!preg_match('/\[learn_press_attendance\s?(.*)\]/', $post->post_content)) {
                    $post->post_content .= '[learn_press_attendance]';
                }
            }
    ...
    public static function recent_courses($atts) {
        ...
        $posts = $wpdb->get_results(
                $wpdb->prepare(
						...
						LIMIT %d
					", LP_COURSE_CPT, 'publish', (int) absint($limit)
                )
        );
    ...
    public static function popular_courses($atts) {
        ...
        $query = $wpdb->prepare(
                "SELECT po.*, count(*) as number_enrolled
					...
					LIMIT %d
				", LP_COURSE_CPT, 'enrolled', 'finished', 'publish', absint($limit)
        );
        ...
        if (!$user) {
            ...
            if (!learn_press_current_user_can_view_profile_section($current, $user)) {
                ...
                if (!empty($tabs) && !empty($tabs[$current])) :
                    learn_press_get_template('profile/index.php', array(
                        'user' => $user,
                        'tabs' => $tabs,
                        'current' => $current
                            )
                    ...
                    if ($wp->query_vars['view'] == LP()->settings->get('profile_endpoints.profile-order-details')) {
                        ...
                        if (!$order) {
                            ...
                            learn_press_get_template('profile/order-details.php', array(
                                'user' => $user,
                                'order' => $order
                                    )
                            );
    ...
    function attendance($atts) {
        ...
        if (!$user) {
            if (empty($wp_query->query['user'])) {
                learn_press_display_message(sprintf(__('Please login to mark attendance', 'learnpress'), $wp_query->query['user']), 'error');
            }
        } else {
            global $wpdb;
            ...
            if (count($courses) > 0) {
                ...
                foreach($courses as $course){
                     $course_start_date = get_post_meta($course['course_id'], "_lp_start_date", true);
                     $course_date = new DateTime($course_start_date);
                     $course_duration_days = intval(get_post_meta($course['course_id'],"_lp_duration", true))-1;
                     $intervalString = 'P' . $course_duration_days . 'D';
                ...
                learn_press_get_template('attendance/mark-attendance.php', array(
                    'user' => $user,
                    'message' => $message,
                ...
                learn_press_display_message(sprintf(__('You dont have permission to mark attendance/ you may not have any courses ', 'learnpress'), $wp_query->query['user']), 'error');
            }
        }
        $output .= ob_get_clean();
LP_Assets::enqueue_script('learn-press-attendance');
        return self::wrapper_shortcode($output);
    }
