...
class LP_Question_Multi_Rank extends LP_Abstract_Question {
    ...
    public static function admin_js_template() {
        ...
        <tr class="lp-list-option lp-list-option-empty <# if(data.id){ #>lp-list-option-{{data.id}}<# } #>" data-id="{{data.id}}">
            ...
            <th class="lp-answer-check">
                <input type="text" name="learn_press_question[{{data.question_id}}][answer][value][]" value="{{data.value}}" />
                <input type="checkbox" style="display: none" name="learn_press_question[{{data.question_id}}][checked][]" checked value="{{data.value}}" />
            </th>
        ...
        <?php
        return apply_filters('learn_press_question_multi_rank_answer_option_template', ob_get_clean(), __CLASS__);
    }
