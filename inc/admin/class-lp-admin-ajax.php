...
//					die( __( 'Course invalid', 'learnpress' ) );
        ...
        public static function add_quiz_question() {
            ...
            if ($id && $quiz_id) {
                ...
                $wpdb->insert(
                        $wpdb->prefix . 'learnpress_quiz_questions', array(
                    'quiz_id' => $quiz_id,
                    'question_id' => $id,
        ...
        public static function change_user_file_status() {
            global $wpdb;
            $newStatus = learn_press_get_request("newAction");
            $upload_id = learn_press_get_request("uploadId");
            $element_pos = learn_press_get_request("elementPosition");
            $result = array("elementPosition" => $element_pos, "uploadId" => $upload_id, "newAction" => $newStatus);
            $upload_table_name = $upload_table_name = $wpdb->prefix . 'learnpress_user_itemuploads';
            ...
            if ($newStatus == "approved") {
                $updated = $wpdb->update($upload_table_name, array("status" => $newStatus), array('upload_id' => $upload_id));
                $where = " WHERE uploads.upload_id = $upload_id ";
                $query = $wpdb->prepare("SELECT  ut.item_id as lesson_id, ut.ref_id as course_id, ut.user_id, ut.item_type FROM $upload_table_name as uploads LEFT JOIN $user_item_table as ut on ut.user_item_id = uploads.learnpress_user_item_id $where ");
                $lesson_detail = $wpdb->get_row($query, ARRAY_A);

                $user = learn_press_get_current_user($lesson_detail['user_id']);
