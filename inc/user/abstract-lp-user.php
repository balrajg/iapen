...
class LP_Abstract_User {
    ...
    private function _create_quiz_history($quiz_id, $course_id = 0/* added 1.0.4 */) {
        ...
        if (empty($this->_quiz_history_id)) {
            ...
            $wpdb->insert(
                    $wpdb->prefix . 'learnpress_user_items', apply_filters('learn_press_user_quiz_history_data', array(
                'user_id' => $this->id,
                'item_id' => $quiz_id,
                'item_type' => 'lp_quiz',
                'ref_id' => $course_id,
                ...
                'status' => ''
                            ), $this->id, $quiz_id, $course_id), array('%d', '%d', '%s', '%d', '%s', '%s', '%s')
            );
            ...
            @$wpdb->update(
                            ...
                            array(
                        'ref_id' => $course_id
                            ), array('user_quiz_id' => $user_quiz_id), array('%d'), array('%d')
            );
            //
            ...
            $user_quiz_data = apply_filters(
                    'learn_press_user_quiz_data', array(
...
//			$current_time = current_time( 'timestamp' );
    ...
    public function get_quiz_status($quiz_id, $course_id = 0, $force = false) {
        ...
        if ((!array_key_exists($this->id . '-' . $course_id . '-' . $quiz_id, $cached) || $force ) && $quiz_ids = $this->get_course_items('lp_quiz', 'key')) {
            ...
            $args = array_merge(
                    array($this->id, $course_id), $quiz_ids
            );
            $query = $wpdb->prepare("
    ...
    public function get_lesson_status($lesson_id, $course_id = 0, $force = false) {
        ...
        if ((!array_key_exists($this->id . '-' . $course_id . '-' . $lesson_id, $cached) || $force ) && $lesson_ids = $this->get_course_items('lp_lesson', 'key')) {
            ...
            $args = array_merge(
                    array($this->id, $course_id), $lesson_ids
            );
            $query = $wpdb->prepare("
				SELECT item_id as id, `status`
				FROM {$wpdb->prefix}learnpress_user_items
    ...
    public function get_item_status($item_id, $course_id = 0, $force = false) {
        ...
        if ((!array_key_exists($key, $item_statuses) || $force)) {
            if ($item_ids = $this->get_course_items('', 'key', $course_id)) {
                ...
                $args = array_merge(
                        array($this->id, $course_id), $item_ids
                );
    ...
    public function get_quiz_history($quiz_id, $course_id = 0, $history_id = null, $force = false) {
        ...
        if ((!array_key_exists($key, $cached) || $force ) && $quizzes && in_array($quiz_id, $quizzes)) {
            ...
            $prepare_params = array_merge(
                    array('lp_quiz', $this->id, $course_id), $quizzes
            );
            $query = $wpdb->prepare("
				SELECT *
				FROM $t1 uq
    ...
    public function finish_course($course_id) {
        ...
        if ($course = LP_Course::get_course($course_id)) {
            if (!$this->can('finish-course', $course_id) && 1 == 0) {
                ...
                $updated = $wpdb->update(
                        $wpdb->prefix . 'learnpress_user_items', array(
                    'end_time' => date('Y-m-d H:i:s', $expired - $time < 0 ? $expired : $time),
                    'status' => 'finished'
                        ), array('user_id' => $this->id, 'item_id' => $course_id), array('%s', '%s')
                );
     ...
     * @return bool
     */
    //has-uploaded-document-course
    public function has_uploaded_document_to_course($course_id, $item_id, $force = false) {
        global $wpdb;
        $query = $wpdb->prepare("SELECT count(*) as count FROM {$wpdb->prefix}learnpress_user_itemuploads WHERE status = %s and learnpress_user_item_id = %d", 'uploaded', learn_press_get_user_item_id($this->id, $item_id));
     ...
     * @return bool|WP_Error
     */
    public function upload_document_to_lesson($lesson_id, $course_id = 0, $file = "") {
        global $wpdb;
    ...
    public function get_courses($args = array()) {
        ...
        if (empty($courses[$key])) {
			...
			", $args['user_id'], LP_COURSE_CPT, 'publish', 'draft', $this->id, $args['user_id'], LP_COURSE_CPT, 'publish'
            );
